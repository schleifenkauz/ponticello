    ...
    fun stopRecording() {
        ...
        if (pathOfLastRecording != null) {
            if (YesNoPrompt("Add recorded audio as sample").showDialog(context) == true) {
                val name = NamePrompt(context[SampleRegistry], "Name for sample", "").showDialog(context) ?: return
                val obj = SampleObject.create(context[currentProject], reactiveVariable(name), pathOfLastRecording!!)
