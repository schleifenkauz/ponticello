...
class BusControlSpecPrompt(
    ...
    private var flowType = initialSpec.flow

    private val channelsSpinner = Spinner<Int>(1, 12, initialSpec.channels)

    private val flowTypeSelector = SimpleSearchableListView(FlowType.entries, "Choose flow type")
        .selectorButton(this::flowType)
    private val rateSelector = SimpleSearchableListView(Rate.entries, "Choose rate")
        .selectorButton(this::rate)

    override val content: HBox = HBox(5.0, rateSelector, flowTypeSelector, channelsSpinner).centerChildren()

    override fun makeSpec(): BusControlSpec = BusControlSpec(rate, channelsSpinner.value, flowType)
}